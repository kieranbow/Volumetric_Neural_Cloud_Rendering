		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				while (t < dist_limit)
				{
                    ...
                    total_density += sample_density(sample_position, height_percent) * step_size;
					...
					if (total_density > 0.0f)
					{
						...
						float dstInsideBox = ray_box_dist(box.bound_min, box.bound_max, light_ray).y;
						float lightStepSize = dstInsideBox / 5;
						float lightDensity = 0.0f;
						
						for (int l = 0; l < 5; l++)
						{
							if (lightDensity > 1.0f) break;
							
							light_ray.origin += light_ray.direction * lightStepSize;
							lightDensity += max(0.0f, sample_density(light_ray.origin, height_percent)) * lightStepSize;
						}

						float lightTransmittance = 0.2f + exp(-lightDensity * 0.8f) * -0.2f;

						float cos_theta = dot(light_ray.direction, _WorldSpaceLightPos0.rgb);
						
						total_light += total_density * step_size * transmittance * lightTransmittance * 0.2f + henyey_Greenstein(cos_theta, 0.83f) * -0.5f + henyey_Greenstein(cos_theta, 0.3f) * 0.8f * 0.15f;
						transmittance *= exp(-total_density * step_size * 0.0f);
				...
				float3 cloud_colour = total_light * _LightColor0.rgb;
                return float4(0.0f * transmittance + cloud_colour, 1.0f);
			}
			ENDCG
		}
